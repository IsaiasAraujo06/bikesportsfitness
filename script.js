...
function adicionarProdutos(categoria, id) {
    const secao = document.querySelector(`#${id} .catalogo`);
...
adicionarProdutos("esteiras", "esteiras");
adicionarProdutos("ergometricas", "ergometricas");
adicionarProdutos("estacao", "estacao");
adicionarProdutos("pesolivre","pesolivre");
adicionarProdutos("acessorios","acessorios");
adicionarProdutos("maquinas","maquinas");
adicionarProdutos("bicicletas","bicicletas");
...
function filtrarProdutos() {
    ...
    let lista = document.getElementById("listaProdutos").getElementsByTagName("li");
    let resultadosDiv = document.getElementById("resultadosPesquisa");
    ...
    for (let item of lista) {
      if (item.textContent.toLowerCase().includes(input)) {
        resultados.push(item.textContent);
      }
    }
...
function buscarProdutos(event) {
    ...
    produtos.forEach(produto => {
        ...
        if (nomeProduto.includes(termoBusca)) {
            ...
            produto.querySelector("h2").innerHTML = nomeProduto.replace(regex, "<mark>$1</mark>");
        } else {
            produto.style.display = "none";
        }
    });

    document.getElementById("mensagem-erro").style.display = encontrou ? "none" : "block";
